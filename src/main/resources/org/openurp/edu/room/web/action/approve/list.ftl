...
[@b.grid items=roomApplies var="roomApply"]
	...
	[@b.row]
		...
		[/@]
[#--		[@b.col title="审核部门"  width="11%" property="auditDepart.name" ][#if roomApply.auditDepart.abbreviation??]${roomApply.auditDepart.abbreviation}[#else]${roomApply.auditDepart.name}[/#if][/@]--]
		[@b.col property="borrower.applicant" sortable="false" title="经办(借用人)"  width="12%"]
			[#if ((roomApply.applyBy.name)!'')==((roomApply.borrower.applicant)!'')]${roomApply.applyBy.name}
			[#else]${(roomApply.applyBy.name)!}(${(roomApply.borrower.applicant)!})[/#if]
		[/@]
		[@b.col property="applyAt" title="日期"  width="5%"]${(roomApply.applyAt?string("yyyy-MM-dd"))?default("")}[/@]
