[#ftl]
[@b.head/]
<script language="JavaScript" type="text/JavaScript" src="${base}/static/scripts/roomApply.js"></script>
[#assign cycleName={'1':'天','2':'周','4':'月'}/]
[@b.grid items=roomApplies var="roomApply"]
	[@b.gridbar]
		bar.addItem("${b.text('action.info')}", action.info());
  	  		bar.addItem("审核通过", "remarkAction('${b.url("!departApprove")}')");
  	  		bar.addItem("审核不通过", "remarkAction('${b.url("!departCancel")}')");
  	  		//bar.addItem("${b.text('action.edit')}", "editApply()");
  			//bar.addItem("取消审核", "remarkAction('${b.url("!departCancel")}')");
	...
	[@b.row]
		[@b.boxcol width="5%"/]
		[@b.col property="activity.name" width="12%"]
			[#if roomApply.isApproved!false]
			[@b.a href="!preview?roomApplyIds=${roomApply.id}" target="_blank"]${(roomApply.activity.name?html)!}[/@]
			[#else]
			[@b.a href="!info?roomApplyId=${roomApply.id}"]${(roomApply.activity.name?html)!}[/@]
			[/#if]
		[/@]
		[@b.col property="usage.name"  width="8%" sortable="false"/]
		[@b.col title="借用时间" width="25%"]${(roomApply.applyTime.toString())!}[/@]
		...
		[@b.col property="updatedAt" title="申请时间"  width="10%"]${(roomApply.updatedAt?string("yy-MM-dd HH:mm"))?default("")}[/@]
...
[/@]
<script>
	function remarkAction(url){
		var form = document.searchRoomApplyApproveForm;
    	var ids = bg.input.getCheckBoxValues("roomApply.id");
	 	if (ids == null || ids == "") {
	 		alert("你没有选择要操作的记录！");
	 		return false;
	 	}
    	bg.form.addInput(form, "roomApplyIds", ids);
    	bg.form.addInput(form, "params", "[@htm.queryStr/]");
	    if(url.indexOf('departCancel')>0){
